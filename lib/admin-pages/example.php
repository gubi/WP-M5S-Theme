...
$settings[basename(__FILE__, '.php')] = array(
	'page_title'		=> __( 'User comments', 'm5s' ),
	'menu_name'			=> __( 'Comments', 'm5s' )
);
...
function table_comments_object($name = 'thead') {
	...
	<<?php echo $name; ?>>
		<tr>
			<th scope="col" style="text-align:center;width:40px;"><?php echo __('Post', 'm5s'); ?></th>
			<th scope="col" style="width:110px;"><?php echo __('User', 'm5s'); ?></th>
			<th scope="col"><?php echo __('Comment', 'm5s'); ?></th>
			<th scope="col" style="width:110px;"><?php echo __('Date', 'm5s'); ?></th>
			<th scope="col" style="text-align:center;width:40px;"><?php echo __('View', 'm5s'); ?></th>
			<th scope="col" style="text-align:center;width:70px;"><?php echo __('Moderate', 'm5s'); ?></th>
		</tr>
	</<?php echo $name; ?>>
	<?php
}
...
function example() { //****************************************************************** da cambiare
	...
	if ( !current_user_can( 'manage_options' ) )  { //se non puoi maneggiare
		wp_die( __('You do not have sufficient permissions to access this page.', 'm5s') );
	}
				...
				if(count($comments)) { //se ci sono commenti
					...
					foreach($comments as $comment) { //ciclo commenti
						...
						<tr class="<?php echo $class_alternate; ?>">
							...
							<td class="colspanchange" align="center"><a href="<?php echo admin_url('admin-ajax.php?action=ajax_moderate_comment&id='.(int)$comment->comment_id); ?>" onclick="return confirm('<?php echo __('Are you sure?', 'm5s'); ?>');"><i class="icon-remove"></i></a></td>
						</tr>
						<?php
						$count++;
					} //fine ciclo
					...
					?>
					<tr class="no-items">
						<td class="colspanchange" colspan="6"><?php echo __('No comments found.', 'm5s'); ?></td>
					</tr>
					<?php
				}
				?>
